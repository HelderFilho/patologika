...
<section class="content-header">
    ...
    <?php echo "<?php  \$this->Html->addCrumb(__(' Home'), '/',['i','class'=>'fa fa-dashboard']);\n";
          ...
          printf("              \$this->Html->addCrumb(__('%s').' '.__('%s'));\n", Inflector::humanize($action), $singularHumanName);
          echo "            echo \$this->Html->getCrumbList(['class'=>'breadcrumb','lastClass'=>'active']);?>\n";?>
</section>
<div class="panel panel-default" style="margin: 20px">
    <div class="panel-heading" align="right">
        <section class="content-header">
        <?php echo "<a href=\"#\"><button class=\"btn btn-primary\" title='<?=__('Back')?>' onclick='window.history.back()' data-original-title='<?=__('Back')?>' data-toggle=\"tooltip\" type=\"button\"><i class=\"fa fa-reply fa-fw\"></i></button></a>\n"?>
        ...
        <th class="actions col-xs-1"><?php echo "<?php echo __('Actions'); ?>"; ?></th>
                                        ...
                                        echo "\t<?php foreach (\${$singularVar}['{$alias}'] as \${$otherSingularVar}): ?>\n";
                                                echo "\t\t\t\t\t\t\t\t\t<tr>\n";
                                                    ...
                                                    echo "\t\t\t\t\t\t\t\t\t\t\t\t\t\t<li><a href=\"<?php echo \$this->Html->url(array('controller' => '{$details['controller']}','action' => 'view', \${$otherSingularVar}['{$details['primaryKey']}'])); ?>\"><i class=\"fa fa-eye fa-fw\"></i> <?=__('View');?></a></li>\n";
                                                    echo "\t\t\t\t\t\t\t\t\t\t\t\t\t\t<li><a href=\"<?php echo \$this->Html->url(array('controller' => '{$details['controller']}','action' => 'edit', \${$otherSingularVar}['{$details['primaryKey']}'])); ?>\"><i class=\"fa fa-edit fa-fw\"></i> <?=__('Edit');?></a></li>\n";
