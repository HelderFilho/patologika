...
App::import('Controller', 'Histologia2s');
...
public function salvar($Id, $Nome, $Tipo, $QuantidadeBlocos, $QuantidadeLaminas){
		$this->Histologia1->create();
		
		$this->request->data['Histologia1']['Id'] = $Id;
		$this->request->data['Histologia1']['Nome_Paciente'] = $Nome;		
		$this->request->data['Histologia1']['Tipo'] = $Tipo;		
		$this->request->data['Histologia1']['Numero_Blocos'] = $QuantidadeBlocos;		
		$this->request->data['Histologia1']['Numero_Laminas'] = $Quantidade_Laminas;
		$this->request->data['Histologia1']['Descartado'] = 'N';
		$this->request->data['Histologia1']['Removed'] = 'N';
		$this->Histologia1->save($this->request->data);
	}
 ...
 */
	public function edit($id = null) {
		$Histologia = new Histologia2sController();
		...
		if ($this->request->is(array('post', 'put'))) {
            ...
            $Modified = new DateTime($this->request->data['Histologia1']['Modified']);
            $this->request->data['Histologia1']['Modified'] = $Modified->format('Y-m-d H:i');
            $this->request->data['Histologia1']['Removed']='S';
            	if ($this->Histologia1->save($this->request->data)) {
            	
                         $nome = $this->request->data['Histologia1']['Nome_Paciente'];
                         $numero_blocos = $this->request->data['Histologia1']['Numero_Blocos'];
                         $numero_frascos = $this->request->data['Histologia1']['Numero_Frascos'];
                         $tipo = $this->request->data['Histologia1']['Tipo'];
                         $inclusao = $this->request->data['Histologia1']['Inclusao'];
                         $Histologia->salvar($id, $nome, $tipo, $numero_blocos, $numero_frascos, $inclusao);
