<section class="content-header">
    <h1>
        ...
        <small><?php echo __('View').' '.__('Arquivo'); ?></small>
    </h1>
    <?php  $this->Html->addCrumb(__(' Home'), '/',['i','class'=>'fa fa-dashboard']);
            $this->Html->addCrumb(__('Arquivos'),'/Arquivos');
              $this->Html->addCrumb(__('View').' '.__('Arquivo'));
...
<div class="panel panel-default" style="margin: 20px">
    <div class="panel-heading" align="right">
        ...
        <a href="#"><button class="btn btn-primary" title='<?=__('Back')?>' onclick='window.history.back()' data-original-title='<?=__('Back')?>' data-toggle="tooltip" type="button"><i class="fa fa-reply fa-fw"></i></button></a>
        	...
        	<a title='<?=__('Edit')?>' data-original-title='<?=__('Edit')?>' data-toggle='tooltip' href="<?php echo $this->Html->url(array('action' => 'edit', $arquivo['Arquivo']['Id'])); ?>"><button class="btn btn-primary" type="button"><i class="fa fa-edit fa-fw"></i></button></a>
        	<?php echo $this->Form->postLink('<button title='.__('Delete').' data-original-title='.__('Delete').' data-toggle="tooltip" class="btn btn-primary"><i class="fa fa-trash-o fa-fw"></i></button>',
                                                array('action' => 'delete',  $arquivo['Arquivo']['Id']),
                                                array('escape'=>false),
                                                __('Are you sure you want to delete # %s?',  $arquivo['Arquivo']['Id'])
                                            );?>
        </section>
    </div>
    <div class="nav-tabs-custom">
        <ul class="nav nav-tabs pull-right">
          <li class="active"><a data-toggle="tab" href="#tab_1-1" aria-expanded="false"><?=__("Arquivo")?></a></li>
                    <li class="pull-left header"><i class="ion ion-clipboard"></i><?=__("Data")?></li>
        </ul>
        <div class="tab-content">
          <div id="tab_1-1" class="tab-pane active">
                <div style="margin: 20px">
		<?php if($arquivo['Arquivo']['Id']){?>
		<dt><?php echo __('Id'); ?></dt>
		<dd><?php echo h($arquivo['Arquivo']['Id']); ?></dd>
		<?php }?>
		<?php if($arquivo['Arquivo']['Nome_Paciente']){?>
		<dt><?php echo __('Nome Paciente'); ?></dt>
		<dd><?php echo h($arquivo['Arquivo']['Nome_Paciente']); ?></dd>
		<?php }?>
                </div>
            </div>
                        </div>
